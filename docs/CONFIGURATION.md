...
| — (flag only) | string | `normal` | `--verbosity` | Startup noise dial: `quiet` (engine traces hidden, compact steps), `normal`, `detailed` (steps expand). `/verbosity` switches at runtime; never persisted to this file. |
